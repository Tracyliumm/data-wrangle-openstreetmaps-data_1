...
def key_type(element, keys):
    # search 3 regular expressions to check for certain patterns in the tags and count the numbers
    if element.tag == "tag":
        # YOUR CODE HERE
        key = element.get("k")
        print key
...
# get the number of the each regular expression
def process_map(filename):
    keys = {"lower": 0, "lower_colon": 0, "problemchars": 0, "other": 0}
    # open osm file and iterate through elements
    for _, element in ET.iterparse(filename):
        # count the number of the each regular expression
